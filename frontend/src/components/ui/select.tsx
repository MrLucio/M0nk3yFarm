...
export const SelectButtonTrigger = <T extends ValidComponent = 'button'>(
    ...
    return (
        <SelectPrimitive.Trigger
            ...
            variant="ghost"
            size="sm"
            class={cn('focus-visible:ring-0', local.class)}
            {...rest}
        >
            {local.children}
        </SelectPrimitive.Trigger>
...
export const SelectTrigger = <T extends ValidComponent = 'button'>(
    ...
    return (
        <SelectPrimitive.Trigger
            class={cn(
                'flex h-9 w-full items-center justify-between rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background transition-shadow placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50',
                local.class
            )}
            {...rest}
        >
    ...
    return (
        <SelectPrimitive.Portal>
            <SelectPrimitive.Content
                class={cn(
                    'relative z-50 overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[expanded]:animate-in data-[closed]:animate-out data-[closed]:fade-out-0 data-[expanded]:fade-in-0 data-[closed]:zoom-out-95 data-[expanded]:zoom-in-95',
                    local.class
                )}
                {...rest}
            >
